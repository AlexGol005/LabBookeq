...
class OrderVerificationView(LoginRequiredMixin, View):
    ...
    def get(self, request, str):
        ...
        if i=='1':
            list = Equipment.objects.filter(pointer=self.request.user.profile.userid).filter(measurequipment__newdatedead_date__lte=serdate).exclude(measurequipment__newdatedead_date='') | Equipment.objects.filter(pointer=self.request.user.profile.userid).filter(testingequipment__newdatedead_date__lte=serdate)
        if i=='2':
