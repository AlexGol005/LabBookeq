...
class OrderVerificationView(LoginRequiredMixin, View):
    ...
    def get(self, request, str):
        ...
        if i=='2':
            list = Equipment.objects.filter(pointer=self.request.user.profile.userid).filter(measurequipment__pk=16) | Equipment.objects.filter(pointer=self.request.user.profile.userid).filter(pk=34)
        else:
            list = Equipment.objects.filter(pointer=self.request.user.profile.userid)
