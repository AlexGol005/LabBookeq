...
def export_orderverification_xls(request, object_ids):
    ...
    for col_num in range(len(columnsME)):
        ws1.write(row_num, col_num, columnsME[col_num], style_plain_border)
    ws1.row(row_num).height_mismatch = True
    ws1.row(row_num).height = 3000

    row_num = 1
    for col_num in range(len(columnsHE)):
        ws2.write(row_num, col_num, columnsHE[col_num], style_plain_border)
...
def export_orderverification_FBU__TEST_S__PETERBURG__xls(request, object_ids):
    ...
    ws.col(11).width = 300
        ...
        # данные
    rows1 = MeasurEquipment.objects.filter(equipment__pk__in=q).annotate(  
    num=Value('1'),\
             note=Value('поверка'),\
             cod1=Value(''),).\
    values_list(
    ...
    rows2 = TestingEquipment.objects.filter(equipment__pk__in=q).annotate(
             num=Value('1'),\
