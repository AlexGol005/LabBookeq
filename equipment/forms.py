...
class EquipmentKategoryUpdateForm(forms.ModelForm):
    ...
    kategory = forms.ChoiceField(label='Выберите категорию прибора.\n При смене категории потребуется добавить характеристики СИ/ИО/ВО заново.\n Для смены характеристик в пределах одной категории - поменяйте на другую категорию и потом на нужную категорию: \n характеристики будут удалены и можно будет добаввить новые характеристики', required=False,
                               choices=KATEGORY,
                               widget=forms.Select(attrs={'class': 'form-control'}))
...
class ServiceEquipmentregTEForm(forms.ModelForm):
    """форма для  внесения постоянного описания ТОИР к ИО"""
    descriptiont0 = forms.CharField(label='Объем технического обслуживания ТО 0', max_length=10000,
                           widget=forms.Textarea(attrs={'class': 'form-control',
                                                        'placeholder': 'введите текст записи о приборе'}))
    descriptiont1 = forms.CharField(label='Объем технического обслуживания ТО 1', max_length=10000,
